...
public class WifiAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(mContext).inflate(R.layout.adapter_wifi_list, parent, false);
		...
		holder.ssid.setText(wifiScanResult.SSID);
		if (wifiScanResult.security == WifiScanResult.SECURITY_NONE) {
			if (wifiScanResult.level >= -69) {
				holder.level.setImageResource(R.drawable.ic_wifi_signal_4_light);
			} else if (wifiScanResult.level >= -80) {
				holder.level.setImageResource(R.drawable.ic_wifi_signal_3_light);
			} else if (wifiScanResult.level >= -88) {
				holder.level.setImageResource(R.drawable.ic_wifi_signal_2_light);
			} else if (wifiScanResult.level >= -92) {
				holder.level.setImageResource(R.drawable.ic_wifi_signal_1_light);
			} else {
				holder.level.setImageResource(R.drawable.ic_wifi_signal_0_light);
			}
		} else {
			if (wifiScanResult.level >= -69) {
				holder.level.setImageResource(R.drawable.ic_wifi_lock_signal_4_light);
			} else if (wifiScanResult.level >= -80) {
				holder.level.setImageResource(R.drawable.ic_wifi_lock_signal_3_light);
			} else if (wifiScanResult.level >= -88) {
				holder.level.setImageResource(R.drawable.ic_wifi_lock_signal_2_light);
			} else if (wifiScanResult.level >= -92) {
				holder.level.setImageResource(R.drawable.ic_wifi_lock_signal_1_light);
			} else {
				holder.level.setImageResource(R.drawable.ic_wifi_lock_signal_0_light);
			}
		}

		if (wifiScanResult.status == WifiScanResult.ENABLED) {
			holder.state.setText(R.string.wifi_remembered);
			...
			case WifiScanResult.DISABLED_AUTH_FAILURE:
				holder.state.setText(R.string.wifi_disabled_password_failure);
				break;
			case WifiScanResult.DISABLED_DHCP_FAILURE:
			case WifiScanResult.DISABLED_DNS_FAILURE:
				holder.state.setText(R.string.wifi_disabled_network_failure);
				break;
			case WifiScanResult.DISABLED_UNKNOWN_REASON:
				holder.state.setText(R.string.wifi_disabled_generic);
			}
		} else if (wifiScanResult.status == WifiScanResult.CURRENT) {
			holder.state.setText(R.string.wifi_connected);
		} else if (wifiScanResult.status == WifiScanResult.AUTHENTICATING) {
			holder.state.setText(R.string.wifi_password_authenticating);
		} else if (wifiScanResult.status == WifiScanResult.OBTAINING_IPADDR) {
			holder.state.setText(R.string.wifi_obtaining_ipaddr);
		} else if (wifiScanResult.status == WifiScanResult.CONNECT_FAILURE) {
			holder.state.setText(R.string.wifi_failed_connect);
		} else {
			holder.state.setText(getEncryptionName(wifiScanResult.capabilities));
		}
		return convertView;
	}
	...
	private class ViewHolder {
		...
		public ViewHolder(View view) {
			...
			state = (TextView) view.findViewById(R.id.tv_wifi_state);
			level = (ImageView) view.findViewById(R.id.image_level);
		}
	}

...
public class CallRecordAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ViewHolder holder;
		if (convertView == null) {
			convertView = LayoutInflater.from(mContext).inflate(R.layout.adapter_call_log_list, null);
		...
		holder.roomName.setText(new RoomInfoUT(mCallRecordList.get(position).roomCode).getRoomName(mContext));
		holder.date.setText(mCallRecordList.get(position).beginTime);
		String time = mCallRecordList.get(position).talkTime;
		holder.callTime.setText(time.substring(time.indexOf(":") + 1, time.length()));// 00:00:00截取成00:00
	...
	public class ViewHolder {
		...
		public void setCheck(boolean isChecked) {
			if (isChecked) {
				...
				roomCode.setTextColor(mContext.getResources().getColor(R.color.DarkIndigo));
				roomName.setTextColor(mContext.getResources().getColor(R.color.DarkIndigo));
				callTime.setTextColor(mContext.getResources().getColor(R.color.DarkIndigo));
				date.setTextColor(mContext.getResources().getColor(R.color.DarkIndigo));
			} else {
				roomCode.setTextColor(mContext.getResources().getColor(R.color.DarkDeepGrey));
				roomName.setTextColor(mContext.getResources().getColor(R.color.DarkDeepGrey));
				callTime.setTextColor(mContext.getResources().getColor(R.color.DarkDeepGrey));
				date.setTextColor(mContext.getResources().getColor(R.color.DarkDeepGrey));
			}
		}
	}
}

...
public class CallLogActivity extends BaseActivity implements OnClickListener {
	...
	private synchronized void updateData() {
		ArrayList<CallInfo> infos;
		if (mButtonCurrent == mButtonAccepted) {
			infos = DPFunction.getCallLogList(CallInfo.CALL_IN_ACCEPT);
		} else if (mButtonCurrent == mButtonCalled) {
			infos = DPFunction.getCallLogList(CallInfo.CALL_OUT_ACCEPT | CallInfo.CALL_OUT_UNACCEPT);
		} else {
			infos = DPFunction.getCallLogList(CallInfo.CALL_IN_UNACCEPT);
		...
		for (int i = 0; i < infos.size(); i++) {
			...
			mods.get(i).beginTime = CommonUT.formatTime(infos.get(i).getStartTime());
			mods.get(i).talkTime = CommonUT.formatTime(infos.get(i).getEndTime() - infos.get(i).getStartTime());
	...
	public void onClick(View v) {
		...
		case R.id.btn_delete:
			if (mAdapter.checkID >= 0) {
				...
				dialog.setOnClickListener(new OnDialogClickListener() {
					...
					@Override
					public void onClick() {
						DPFunction.deleteCallLog(mAdapter.getCallRecordList().get(mAdapter.checkID).id);
						if (mAdapter.getCallRecordList().get(mAdapter.checkID).callType == CallInfo.CALL_IN_UNACCEPT
								&& mAdapter.getCallRecordList().get(mAdapter.checkID).isDoor) {
							deleteFile();
						}
			...
			if (mAdapter.getCallRecordList().size() > 0) {
				...
				dialog.setOnClickListener(new OnDialogClickListener() {
					...
					public void onClick() {
						int type = 0;
						if (mButtonCurrent == mButtonAccepted) {
							type = CallInfo.CALL_IN_ACCEPT | CallInfo.CALL_OUT_ACCEPT;
						} else if (mButtonCurrent == mButtonCalled) {
							type = CallInfo.CALL_OUT_ACCEPT | CallInfo.CALL_OUT_UNACCEPT;
						} else {
							type = CallInfo.CALL_IN_UNACCEPT;
		...
		case R.id.btn_call:
			if (mAdapter.checkID >= 0) {
				callOut(mAdapter.getCallRecordList().get(mAdapter.checkID).roomCode);
			} else if (mAdapter.getCallRecordList().size() > 0) {
				MyToast.show(R.string.no_item_check);
			} else {
				MyToast.show(R.string.no_item_to_del);
	...
	private void deleteFile() {
		String picturePath = new String(ConstConf.VISIT_PATH + File.separator
				+ mAdapter.getCallRecordList().get(mAdapter.checkID).beginTime.replaceAll(":", "-") + ".jpg");
		String audioPath = new String(ConstConf.VISIT_PATH + File.separator
				+ mAdapter.getCallRecordList().get(mAdapter.checkID).beginTime.replaceAll(":", "-") + ".wav");
		...
		picturePath = new String(ConstConf.VISIT_LITTLE_PATH + File.separator
				+ mAdapter.getCallRecordList().get(mAdapter.checkID).beginTime.replaceAll(":", "-") + ".jpg");
		audioPath = new String(ConstConf.VISIT_PATH + File.separator
				+ mAdapter.getCallRecordList().get(mAdapter.checkID).beginTime.replaceAll(":", "-") + ".silk");

...
public class SMSActivity extends BaseActivity implements OnClickListener {
	...
	private void updateCurrentList(Button button) {
		...
		if (mButtonCurrent == mButtonCommunity) {
			...
			datas = DPFunction.getMessageLogList(MessageInfo.PERSONAL | MessageInfo.PUBLIC);
		}
		mAdapter.checkPos = -1;
	...
	public void onResume() {
		...
		PhysicsKeyService.setKeySwitch(new boolean[] { false, true, true, false, false });
		super.onResume();
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_delete:
			if (mAdapter.checkPos >= 0) {
				...
				dialog.setOnClickListener(new OnDialogClickListener() {
					...
					@Override
					public void onClick() {
						DPFunction.deleteMessageLog(mAdapter.messageList.get(mAdapter.checkPos).getDb_id());
						String resName = mAdapter.messageList.get(mAdapter.checkPos).getResName();
						if (resName != null) {
							File file = new File(ConstConf.MESSAGE_PATH + File.separator + resName);
							if (file.exists()) {
								file.delete();
							}
							file = new File(ConstConf.MESSAGE_LITTLE_PATH + File.separator + resName);
							if (file.exists()) {
								file.delete();
							}
						}
				...
				dialog.setOnClickListener(new OnDialogClickListener() {
					...
					public void onClick() {
						if (mButtonCurrent == mButtonCommunity) {
							...
							DPFunction.deleteAllMessageLog(MessageInfo.PERSONAL | MessageInfo.PUBLIC);
		...
		case R.id.btn_check:
			if (mAdapter.checkPos >= 0) {
				...
				DPFunction.modifyMessageLog(mAdapter.messageList.get(mAdapter.checkPos));
				mAdapter.notifyDataSetChanged();
				String resName = mAdapter.messageList.get(mAdapter.checkPos).getResName();
				if (resName == null) {
	...
	private void deleteFile() {
		if (mAdapter.messageList != null && mAdapter.messageList.size() > 0) {
			for (int i = 0; i < mAdapter.messageList.size(); i++) {
				String resName = mAdapter.messageList.get(i).getResName();
				if (resName != null) {
					File file = new File(ConstConf.MESSAGE_PATH + File.separator + resName);
					if (file.exists()) {
						file.delete();

 ...
 * 工程設置----防区设置
 */
public class DefenceAreaSettingActivity extends BaseActivity implements OnClickListener, OnItemClickListener {
	...
	private class AlarmInfoAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			ItemHolder holder;
			if (convertView == null) {
				convertView = LayoutInflater.from(mContext).inflate(R.layout.adapter_area_list, null);
			...
			holder.connection.setText(mInfoList.get(position).open ? R.string.level_high : R.string.level_low);
			long abnormal = mOldState ^ mConnection;
			if ((abnormal & (1 << position)) > 0) {
				holder.isAbnormal.setBackgroundResource(R.drawable.img_defence_area_detail_check_red);
			} else {
				holder.isAbnormal.setBackgroundResource(R.drawable.img_defence_area_detail_check_green);
			}
			return convertView;
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == 100 && resultCode == 101) {
			...
			mAreaArray[id] = area;
			mTypeArray[id] = type;
			mInfoList.get(id).areaName = DPFunction.getAlarmAreaNameList().get(area).name;
			mInfoList.get(id).areaType = DPFunction.getAlarmTypeNameList().get(type).name;

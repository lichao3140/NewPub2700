 ...
 * WiFi设置
 */
public class WifiActivity extends BaseFragmentActivity implements OnClickListener {
	...
	private WifiCallback mCallback = new WifiCallback() {
		...
		public void updateWifiList() {
			mAdapter.wifiList = wifiSettings.getWifiList();
	...
	private void setListViewListener() {
		mListView.setOnItemClickListener(new ListView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (mAdapter.wifiList.get(position).status == WifiScanResult.ENABLED
						|| mAdapter.wifiList.get(position).status == WifiScanResult.DISABLED
						|| mAdapter.wifiList.get(position).status == WifiScanResult.CONNECT_FAILURE) {
				...
				} else if (mAdapter.wifiList.get(position).status == WifiScanResult.CURRENT) {
					int security = mAdapter.wifiList.get(position).security;
					int ipType = mAdapter.wifiList.get(position).ipType;
					showWifiInfo(security, ipType);
				} else if (mAdapter.wifiList.get(position).status == WifiScanResult.AUTHENTICATING
						|| mAdapter.wifiList.get(position).status == WifiScanResult.OBTAINING_IPADDR) {
	...
	private synchronized void wifiControl(String ssid, int security, int netId, int ipType) {
		if (mModifyDialog.isAdded() || mInfoDialog.isAdded() || mConnectDialog.isAdded() || mControlDialog.isAdded()) {
			return;
		}
		mControlDialog = new WifiControlDialog();
		setControlDialogListener();
	...
	private synchronized void showWifiInfo(int security, int ipType) {
		if (mModifyDialog.isAdded() || mInfoDialog.isAdded() || mConnectDialog.isAdded() || mControlDialog.isAdded()) {
			return;
		}
		mInfoDialog = new WifiInfoDialog();
		setInfoDialogListener();
		mWifiInfo = wifiSettings.getConnectionInfo();
		...
		mInfoDialog.ip = CommonUT.intToString(mWifiInfo.getIpAddress());
		mInfoDialog.security = security;
		mInfoDialog.ipType = ipType;
		DhcpInfo mDhcpInfo = wifiSettings.getDhcpInfo();
		if (mDhcpInfo != null) {
			String subnetMask = CommonUT.intToString(mDhcpInfo.netmask);
			...
			mInfoDialog.gateway = CommonUT.intToString(mDhcpInfo.gateway);
			mInfoDialog.dns = CommonUT.intToString(mDhcpInfo.dns1);
		} else {
	...
	private synchronized void connectWifi(String ssid, int security) {
		if (security == WifiScanResult.SECURITY_NONE) {
			wifiSettings.connectWifi(ssid, "", security);
		} else {
			if (mModifyDialog.isAdded() || mInfoDialog.isAdded() || mConnectDialog.isAdded()
					|| mControlDialog.isAdded()) {
				return;
			}
			mConnectDialog = new WifiConnectDialog();
			setConnectDialogListener();
	...
	private void setControlDialogListener() {
		mControlDialog.setControlDialogClickListener(new OnControlDialogClickListener() {

			@Override
			public void onRemoveNetwork(int netId) {
				wifiSettings.removeNetwork(netId);
			}
			...
			public void onConnectNetwork(String ssid) {
				wifiSettings.connectWifi(ssid, null, 0);
			}
		});
	}

	private void setInfoDialogListener() {
		mInfoDialog.setInfoDialogClickListener(new OnInfoDialogClickListener() {
			...
			public void onDisconnectNetwork() {
				wifiSettings.disconnectWifi();
			}
		});
	}

	private synchronized void modifyNetwork(String ssid, int security, int netId, int ipType) {
		if (mModifyDialog.isAdded() || mConnectDialog.isAdded()) {
			return;
		}
		mModifyDialog = new WifiModifyDialog();
		setModifyDialogListener();
	...
	private void setConnectDialogListener() {
		mConnectDialog.setOnConnectDialogClickListener(new OnConnectDialogClickListener() {
			...
			@Override
			public void onStaticIpConnect(final String ssid, final String password, final int security,
					final String ipAddress, final String dnsAddress, final String gateway) {
				Thread thread = new Thread() {

					@Override
					public void run() {
						int result = wifiSettings.staticIpConnect(ssid, password, security, ipAddress, dnsAddress,
								gateway);
			...
			public void onConnectWifi(String ssid, String password, int security) {
				wifiSettings.connectWifi(ssid, password, security);
			}
		});
	}
	...
	private void setModifyDialogListener() {
		mModifyDialog.setOnModifyDialogClickListener(new OnModifyDialogClickListener() {
			...
			@Override
			public void onSetStaticIp(final int netId, final String ipAddress, final String dnsAddress,
					final String gateway) {
				Thread thread = new Thread() {

					@Override
					public void run() {
						int result = wifiSettings.setStaticIp(netId, ipAddress, dnsAddress, gateway);
						if (result == WifiSettings.FAILURE) {
							mHandler.sendEmptyMessage(SET_STATIC_IP_FAILED);
						}
					}
			...
			public void onSetDHCP(final int netId) {
				Thread thread = new Thread() {
					...
					@Override
					public void run() {
						int result = wifiSettings.setDHCP(netId);
						if (result == WifiSettings.FAILURE) {
							mHandler.sendEmptyMessage(SET_STATIC_IP_FAILED);
						}
					}
	...
	public void onClick(View v) {
		...
		case R.id.btn_switch:
			...
			mSwitch.setEnabled(false);
			if (!wifiSettings.setWifiEnabled(isChecked)) {
				// Error
				mSwitch.setEnabled(true);
				mIsWifiError = true;
				MyToast.show(R.string.wifi_error);

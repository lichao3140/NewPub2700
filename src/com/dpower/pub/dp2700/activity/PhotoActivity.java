 ...
 * @Funtion 娱乐天地/相册浏览
 */
public class PhotoActivity extends BaseFragmentActivity implements OnClickListener {

	private final String SYSTEM_PATH = "system/media/image/";
	private final String SYSTEM_BACKUP_PATH = "system/media/backup/wall_backgroud/";
	...
	private void initData(List<URI> datas) {
		...
		scanPath(SYSTEM_BACKUP_PATH, datas);
	...
	private void scanPath(String path, List<URI> datas) {
		...
		for (int i = 0; i < subFile.length; i++) {
			if (!subFile[i].isDirectory()) {
				...
				if (name.endsWith(".jpg") || name.endsWith(".png") || name.endsWith(".mpeg") || name.endsWith(".bmp")) {
					datas.add(subFile[i].toURI());
				}
			} else {
				scanPath(subFile[i].getAbsolutePath(), datas);
			}
		}
	}
	...
	private class ImageAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup view, int position) {
			View imageLayout = mInflater.inflate(R.layout.adapter_photo_list, view, false);
			assert imageLayout != null;
			ImageView imageView = (ImageView) imageLayout.findViewById(R.id.image);
			final ProgressBar spinner = (ProgressBar) imageLayout.findViewById(R.id.progress_bar);
			String url = mDatas.get(position).toString().replace("file:/", "file://");
	...
	public void onClick(View v) {
		...
		case R.id.btn_confirm:
			SPreferences.getInstance().saveWallpaper(new File(mDatas.get(mCurrentPosition)).getAbsolutePath());
			finish();
			break;
		default:
			break;

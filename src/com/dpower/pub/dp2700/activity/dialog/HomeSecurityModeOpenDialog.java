 ...
 * 启动安防模式确定窗口
 */
public class HomeSecurityModeOpenDialog extends BaseActivity implements OnClickListener {
	...
	private void init() {
		...
		mModeflag = getIntent().getIntExtra(ConstConf.SAFE_MODE, ConstConf.UNSAFE_MODE);
		String string = null;
		switch (mModeflag) {
	...
	public void onClick(View v) {
		...
		case R.id.btn_confirm:
			if (mModeflag == ConstConf.UNSAFE_MODE) {
				...
				disalarm.setAction(CheckPasswordDialog.DISALARM);
				startActivity(disalarm);
			} else {
				Intent intent = new Intent(mContext, HomeSecurityModeDelayDialog.class);
				intent.putExtra(ConstConf.SAFE_MODE, mModeflag);

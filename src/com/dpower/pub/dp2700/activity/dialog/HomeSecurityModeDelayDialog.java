...
public class HomeSecurityModeDelayDialog extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDelayTime += 1;
		mClickedMode = getIntent().getIntExtra(ConstConf.SAFE_MODE, ConstConf.UNSAFE_MODE);
		...
		mTimeCountRunnable = new Runnable() {
			...
			public void run() {
				...
				if (mDelayTime > 1) { // 延迟 最后1秒用于显示提示
					...
					((TextView) findViewById(R.id.tv_delay_time)).setText(String.valueOf(mDelayTime));
					if (mMediaPlayer == null) {
						mMediaPlayer = MediaPlayer.create(HomeSecurityModeDelayDialog.this, R.raw.timeover15);
					}
					mMediaPlayer.setLooping(false);
					mMediaPlayer.start();
				} else if (mDelayTime > 0) { // 不延迟
					((TextView) findViewById(R.id.tv_mode_type)).setText(getString(R.string.tips_mode_has_started));
					((TextView) findViewById(R.id.tv_delay_time)).setVisibility(View.INVISIBLE);
					...
					case ConstConf.LEAVE_HOME_MODE:
						...
						break;
					case ConstConf.UNSAFE_MODE:
						startActivity(new Intent(HomeSecurityModeDelayDialog.this, HomeSecurityCancelTipsDialog.class));
						break;
					}

 ...
 * 智能家居
 */
public class IntelligentHomeFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (ProjectConfigure.project == 1) {
			mRootView = inflater.inflate(R.layout.fragment_intelligent_home1, container, false);
		} else if (ProjectConfigure.project == 2) {
			mRootView = inflater.inflate(R.layout.fragment_intelligent_home2, container, false);
		} else {
			mRootView = inflater.inflate(R.layout.fragment_intelligent_home, container, false);
		}
		init();
		return mRootView;
	}
	...
	private void init() {
		...
		mButtonArray = new Button[] { mAtHome, mInBed, mDinner, mMedia, mEntertainment, mDisableAll };
		for (int i = 0; i < mButtonArray.length; i++) {
			mButtonArray[i].setOnClickListener(this);
		}
		mButtonSmart = (Button) mRootView.findViewById(R.id.btn_smart_home_server_on);
	...
	private void setButtonBackground() {
		...
		for (int i = 0; i < mButtonArray.length; i++) {
			if (i == mFlag) {
				if (ProjectConfigure.project == 1) {
					mButtonArray[i].setBackgroundResource(R.drawable.bg_home_orange_selector);
				} else {
					mButtonArray[i].setBackgroundResource(R.color.color_button_transparency_purple);
				}
			} else {
				if (ProjectConfigure.project == 1) {
					mButtonArray[i].setBackgroundResource(R.drawable.bg_home_blue_selector);
				} else {
					mButtonArray[i].setBackgroundResource(R.color.color_button_blue_purple);
				}
			}
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_at_home:
			intent.putExtra("mode", SmartHomeService.ACTION_IN_HOME);
			mFlag = 0;
			break;
		case R.id.btn_in_bed:
			intent.putExtra("mode", SmartHomeService.ACTION_IN_BED);
			mFlag = 1;
			break;
		case R.id.btn_dinner:
			intent.putExtra("mode", SmartHomeService.ACTION_IN_DINNER);
			mFlag = 2;
			break;
		case R.id.btn_media:
			intent.putExtra("mode", SmartHomeService.ACTION_IN_VIDEO);
			mFlag = 3;
			break;
		case R.id.btn_entertainment:
			intent.putExtra("mode", SmartHomeService.ACTION_IN_ENTERTAINMENT);
			mFlag = 4;
			break;
		case R.id.btn_disable_all:
			intent.putExtra("mode", SmartHomeService.ACTION_ALL_OFF);
			mFlag = 5;
			break;
		case R.id.btn_smart_home_server_on:
			if (SPreferences.getInstance().getSmartServerState()) {
				intent.putExtra("mode", SmartHomeService.ACTION_SERVER_OFF);
			} else {
				intent.putExtra("mode", SmartHomeService.ACTION_SERVER_START);
			}
			break;
		default:
			break;

...
public class VisualIntercomFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (ProjectConfigure.project == 1) {
			mRootView = inflater.inflate(R.layout.fragment_visual_intercom1, container, false);
		} else if (ProjectConfigure.project == 2) {
			mRootView = inflater.inflate(R.layout.fragment_visual_intercom2, container, false);
		} else {
			mRootView = inflater.inflate(R.layout.fragment_visual_intercom, container, false);
		}
		init();
		return mRootView;
	}
	...
	private void updateView() {
		...
		for (int i = 0; i < callLogList.size(); i++) {
			...
			if (callLogList.get(i).isDoor() && !callLogList.get(i).isRead()) {
				messageCount++;
			}
		}
		if (callLogCount > 0) {

...
import com.dpower.pub.dp2700.activity.dialog.HomeSecurityModeOpenDialog;
...
public class MainFragment extends Fragment implements OnClickListener {
	...
	private void registerReceiver() {
		...
		filter.addAction(ConstConf.VOLUME_CHANGED_ACTION);
	...
	private void updateVolumeView() {
		int vol = mAudioManager.getStreamVolume(AudioManager.STREAM_SYSTEM);
		if (vol == 0) {
	...
	private void updateMessageView() {
		int count = DPFunction.getUnReadMessageLogNum(MessageInfo.PERSONAL | MessageInfo.PUBLIC);
		if (count > 0) {
	...
	private void updateCallLogView() {
		ArrayList<CallInfo> callLogList = DPFunction.getCallLogList(CallInfo.CALL_IN_UNACCEPT);
		if (callLogList == null) {
	...
	public void onClick(View v) {
		...
		case R.id.btn_ring_enable:
			int beforeVol = mAudioManager.getStreamVolume(AudioManager.STREAM_SYSTEM);
			if (beforeVol == 0 || (mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT)) {
				mAudioManager.setRingerMode(AudioManager.RINGER_MODE_NORMAL);
				...
				if (afterVol == 0) {
					...
					mAudioManager.setStreamVolume(AudioManager.STREAM_SYSTEM, afterVol, AudioManager.FLAG_PLAY_SOUND);
				}
				beforeVol = mAudioManager.getStreamVolume(AudioManager.STREAM_SYSTEM);
				updateVolumeView();
			} else {
		...
		case R.id.btn_out_security:
			...
			intent.setClass(getActivity(), HomeSecurityModeOpenDialog.class);
			if (DPFunction.getSafeMode() == ConstConf.UNSAFE_MODE) {
				if (DPFunction.getAlarming()) {
					MyToast.show(R.string.alarm_alarming_to_unsafe);
					break;
				}
				intent.putExtra(ConstConf.SAFE_MODE, ConstConf.LEAVE_HOME_MODE);
		...
		case R.id.btn_call_elevator:
			...
			if (arrayList != null) {
				...
				if (ProjectConfigure.project == 2) {
					MyToast.show(R.string.tips_called_elevator);
				} else {
					if (result == 0) {

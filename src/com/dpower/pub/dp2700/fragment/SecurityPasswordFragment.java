...
public class SecurityPasswordFragment extends BaseFragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		mRootView = inflater.inflate(R.layout.fragment_security_password, container, false);
		init();
		return mRootView;
	}
	...
	private void changePassword() {
		if (checkInput()) {
			if (mIsHolding) { // 修改挟持密码
				...
				if (DPFunction.getSafePassword(false).equals(mNewPassword.getText().toString())) {
					MyToast.show(R.string.seized_cannot_same_as_cancel);
				} else {
					if (DPFunction.setSafePassword(true, mNewPassword.getText().toString())) {
				...
				if (DPFunction.getSafePassword(true).equals(mNewPassword.getText().toString())) {
					MyToast.show(R.string.seized_cannot_same_as_cancel);
				} else {
					if (DPFunction.setSafePassword(false, mNewPassword.getText().toString())) {
						MyToast.show(R.string.change_succeeded);
					} else {
						MyToast.show(R.string.change_failed);
					}
	...
	private boolean checkInput() {
		if (mOldPassword.getText().toString().length() != 6 || mNewPassword.getText().toString().length() != 6

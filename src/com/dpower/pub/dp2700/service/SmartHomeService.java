	...
	public void onCreate() {
		...
		if (CommonUT.getLanConnectState(ConstConf.WAN_NETWORK_CARD)) {
			...
			if (!wifiManager.isWifiEnabled()) {
				...
				mWlanIp = CommonUT.intToString(ipAddress);
			}
			...
			if (action.equals(ACTION_SMART_HOME)) {
				...
				try {
					if (mode.equals(ACTION_ALL_OFF)) {
						...
						if (mSocketThread != null && mClientSocket != null && mFlag) { // 处于运行态
							SPreferences.getInstance().saveSmartServerState(true);
							sendBroadcast(new Intent(DPFunction.ACTION_SMART_HOME_MODE));
							return;
						} else if (mSocketThread != null && mClientSocket != null && !mFlag) { // socket未关闭但已停止接收或者不处理接受
							newReceiverThread();
	...
	private class SocketThread extends Thread {
		...
		public void run() {
			try {
				...
				InetSocketAddress localAddress = new InetSocketAddress(InetAddress.getByName(mWlanIp), 30011);
				...
				if (mClientSocket != null) {
					...
					MyLog.print(TAG, "mClientSocket host adds:" + mClientSocket.getLocalAddress().getHostAddress());
					MyLog.print(TAG, "mClientSocket local port:" + mClientSocket.getLocalPort());
				}
			}
			super.run();
		}
